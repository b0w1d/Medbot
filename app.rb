...
def get_args_freq_line(options = {})
  xname = /(?:x\s*|axis|around)+\s*(?::|\-|upon|is|in|on|by|at|for|with|as|\s*)\s*([^\s]+)\s*/i.match(options[:msg])[1] rescue nil
...
def get_args_freq_bar(options = {})
  xname = /(?:group|bar\s*|categorize|categorized)+\s*(?:by|on|\s*)\s*([^\s]+)\s*/i.match(options[:msg])[1] rescue nil
  xname = format_label(xname)
